...
export const getDelegateList = (options = {}) => {
  ...
  cdo.setLongValue('lCollectEmpId', getStore('userId')); // 催收员idid
  cdo.setStringValue('strLat', options.strLat || '39.91');//纬度
  cdo.setStringValue('strLon', options.strLon || '116.48');//经度
  ...
  options.nFilter && cdo.setIntegerValue('nFilter', options.nFilter); //筛选：-1:默认;0:距离由近到远;1:倒计时由长到短;2:倒计时由短到长;3:金额由大到小;4:逾期由长到短;5:逾期由短到长;
...
export const followOrder = (options = {}) => {
  ...
  cdo.setLongValue('lCollectEmpId', getStore('userId')); // 催收员id
...
export const addReminder = (options = {}) => {
   ...
   * strContact: 联系方式
   */
  !isNull(options.cdosContract) && cdo.setCDOArrayValue('cdosContract', getCdoArr(options.cdosContract, ['nContactType', 'strContact'])); //
  /**
   * 地址数组，在cdoRequest层级内

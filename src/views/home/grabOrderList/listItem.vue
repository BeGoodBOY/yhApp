  ...
  <li class="list-item">
    <template>
      <h3 class="item-header">
        {{ order.strName }}
        <span v-if="order.nIsNew != 1"
          >出库倒计时：{{ caculateDay(order.dtEntrustExpireDate) }}天</span
        >
      </h3>
      <div class="item-body">
        <div class="item-money">
          <div class="money-val">
            {{ formatMoney(order.lShowShouldRefundTotal / 100) }}元
          </div>
      ...
      <div class="item-footer">
        <div class="footer-l">
          <span class="address-key"
            >{{ getAdressTit(order.nAddressType) }}：</span
          >
